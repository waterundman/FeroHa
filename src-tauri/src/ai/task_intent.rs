...
impl TaskIntentType {
    pub fn parse(value: &str) -> Option<Self> {
        match value.trim().to_ascii_lowercase().replace('-', "_").as_str() {
            ...
            "mdt_index" | "index_mdt" => Some(Self::MdtIndex),
            "mdt_read" | "read_mdt" => Some(Self::MdtRead),
            "mdt_pack" | "pack_mdt" => Some(Self::MdtPack),
            "write_proposal" | "ghost_write" => Some(Self::WriteProposal),
    ...
    pub fn default_bridge_risk(self) -> BridgeRisk {
        match self {
            TaskIntentType::Summarize | TaskIntentType::Verify | TaskIntentType::MdtRead => {
                BridgeRisk::Low
            }
