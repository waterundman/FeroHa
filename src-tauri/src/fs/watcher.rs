...
impl FileWatcher {
    ...
    pub fn watch<P: AsRef<Path>>(vault_path: P) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let mut watcher: Box<dyn Watcher> = Box::new(notify::recommended_watcher(move |res: Result<Event, notify::Error>| {
            if let Ok(event) = res {
